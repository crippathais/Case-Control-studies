...
readAndRun <- function(label,ann.tb){
  ...
  colnames(epimirna) = c("sex",sprintf("PC%d", 1:10),"status") 
  ...
  runSKAT <- function(map,data,epimirna,genelist,ann.sub,assoc.out){
    ...
    formula = status ~ sex +PC1 +PC2 +PC3 +PC4 +PC5 +PC6 +PC7 +PC8 +PC9 +PC10
    
    ##### MODELO NULO
    #epim <-SKAT_Null_Model(formula, data=epimirna, out_type="D")
    epimx <-SKAT_Null_Model_ChrX(formula,SexVar="sex", out_type="D", data=epimirna)
    ...
    # Outputs for ChrX
    #df     = data.frame(gene=character(),linear.weighted=double(),IBS =double(),IBS.weighted =double(),quadratic =double(),TwoWayIX =double(),SKATO =double(),nsnps= integer(),snps=character(),stringsAsFactors = FALSE)
    #df.adj = data.frame(gene=character(),linear.weighted=double(),IBS =double(),IBS.weighted =double(),quadratic =double(),TwoWayIX =double(),SKATO =double(),nsnps= integer(),snps=character(),stringsAsFactors = FALSE)
    #colnames(df)[6] <- "2wayIX"
    #colnames(df.adj)[6] <- "2wayIX"
    #assoc.out   = paste("epimirna/skat/",tail(filename,1),".chrX.c",c,sep = "")
    ...
    for (i in 1:length(genelist)) {
      ...
      # xsnps = map[map$CHR == 23,]$ID
      # data.sub = as.matrix(data[,colnames(data) %in% xsnps])
      ...
      if(ncol(data.sub)>0){
          print(paste("i=",i,"/",total_loop,"ChrX"))
          epiSKAT = SKAT_ChrX(data.sub, epimx, method="SKATO", kernel = kernels.SKATO[1])
          df[idx,"SKATO"] = epiSKAT$p.value 
    ...
    ########################
    # Total genelist:
    print(paste("Total ChrX genes:",nrow(df)))
    ...
    # Results for Chr X
    ########################
    # print(paste("Total chrX genelist:",nrow(df)))
    # # p-value correction
    # # how many genes with significant p-val
    # nsigp    = nrow(df[df$linear.weighted<0.05,])
    # print(paste("pval<0.05:",nsigp))
    # # Adjust p-value
    # for(k in 1:length(kernels)){
    #   df.adj[1:nrow(df),kernels[k]] = p.adjust(df[,kernels[k]], method = "bonferroni")
    # }  
    # df.adj[,"SKATO"] = p.adjust(df[,"SKATO"], method = "bonferroni")
    # df.adj$gene = df$gene
    # df.adj$snps = df$snps
    # df.adj$nsnps= df$nsnps
    # # how many genes with significant adjusted p-val
    # nsigpadj = nrow(df.adj[df.adj$linear.weighted<0.05,])
    # print(paste("pval<0.05 bonferroni:",nsigpadj))
    # #[1] 
    # #[1] 
    # 
    # df.ordered = df[order(df$linear.weighted),]
    # df.adj.ordered = df.adj[order(df.adj$linear.weighted),]
    # 
     print(paste("Results for chrX writen to:",assoc.out))
     write.table(df.ordered, file = assoc.out, sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
     write.table(df.adj.ordered, file = paste(assoc.out,".adj",sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
    # 
    end.time <- Sys.time()
  ...
  print(assoc.out)
  #assoc.out   = paste(outputdir,tail(filename,1),".chrX",sep = "")
